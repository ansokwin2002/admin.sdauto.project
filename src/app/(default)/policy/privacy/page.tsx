...
export default function PrivacyPolicyPage() {
  ...
  const fetchCurrent = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          // No existing policies, start with empty form
          setCurrentId(null);
      ...
      const item = data[0]; // Get first policy record

      if (item) {
        setCurrentId(item.id);
        setTitle(item.title || 'Privacy Policy');
        setContent(item.privacy || '');
      } else {
        setCurrentId(null);
        setTitle('Privacy Policy');
        setContent('');
      ...
      toast.error(e.message || 'Failed to load privacy policy');
    } finally {
      setLoading(false);
      NProgress.done();
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        toast.error(json?.message || 'Failed to save privacy policy');
        return;
      }

      toast.success(json?.message || 'Privacy policy saved successfully');
  ...
  return (
    <Box className="w-full">
      <PageHeading title="Privacy Policy" description="Manage your privacy policy content" />

      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        <Card className="!overflow-visible">
          <Box p="4">
            <Text as="div" size="3" weight="bold" mb="4">Privacy Policy Settings</Text>
